...
class Ftp {
    ...
    add_filter( 'futurewordpress/project/aicontentgenerator/filesystem/mediadirectory', [ $this, 'getAllLocalFiles' ], 10, 2 );
		// add_action( 'init', [ $this, 'deleteFiles' ], 10, 0 );
	}
	public function initialize() {
    $this->lastError        = false;
    $this->ftpConnect       = false;
    $this->ftpServer        = apply_filters( 'futurewordpress/project/aicontentgenerator/system/getoption', 'ftp-server', false );
    $this->ftpUsername      = apply_filters( 'futurewordpress/project/aicontentgenerator/system/getoption', 'ftp-username', false );
    $this->ftpPassword      = apply_filters( 'futurewordpress/project/aicontentgenerator/system/getoption', 'ftp-password', false );
    $this->ftpDirectory     = apply_filters( 'futurewordpress/project/aicontentgenerator/system/getoption', 'ftp-remotedir', false );
    $this->localDirectory   = apply_filters( 'futurewordpress/project/aicontentgenerator/system/getoption', 'ftp-localdir', false );
    if( $this->localDirectory === false || empty( $this->localDirectory ) ) {
      $this->localDirectory = apply_filters( 'futurewordpress/project/aicontentgenerator/filesystem/uploaddir', false );
    }
	}
  public function syncFiles() {
    // if( ! apply_filters( 'futurewordpress/project/aicontentgenerator/system/isactive', 'ftp-enable' ) ) {return;}
