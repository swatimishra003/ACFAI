 ...
 */
 class Helpers {
	...
	protected function setup_hooks() {
		if( ! defined( 'FUTUREWORDPRESS_PROJECT_AICONTENTGENERATOR_OPTIONS' ) ) {define( 'FUTUREWORDPRESS_PROJECT_AICONTENTGENERATOR_OPTIONS', (array) get_option( 'ai-content-generator-on-acf-field', [] ) );}
		add_filter( 'futurewordpress/project/aicontentgenerator/system/getoption', [ $this, 'getOption' ], 10, 2 );
		add_filter( 'futurewordpress/project/aicontentgenerator/system/isactive', [ $this, 'isActive' ], 10, 1 );

		add_filter( 'futurewordpress/project/aicontentgenerator/database/countries', [ $this, 'databaseCountries' ], 10, 2 );
		add_filter( 'futurewordpress/project/aicontentgenerator/database/countryflags', [ $this, 'countryFlags' ], 10, 1 );

		add_filter( 'futurewordpress/project/aicontentgenerator/user/dashboardpermalink', [ $this, 'dashboardPermalink' ], 10, 2 );
		add_filter( 'futurewordpress/project/aicontentgenerator/user/visitorip', [ $this, 'visitorIP' ], 10, 0 );


		add_filter( 'futurewordpress/project/aicontentgenerator/filter/server/time', [ $this, 'serverTime' ], 10, 2 );
		add_filter( 'futurewordpress/project/aicontentgenerator/filter/string/random', [ $this, 'generateRandomString' ], 10, 2 );

		add_filter( 'futurewordpress/project/aicontentgenerator/filesystem/filemtime', [ $this, 'filemtime' ], 10, 2 );
		add_filter( 'futurewordpress/project/aicontentgenerator/filesystem/uploaddir', [ $this, 'uploadDir' ], 10, 2 );
		add_filter( 'futurewordpress/project/aicontentgenerator/mailsystem/sendmail', [ $this, 'sendMail' ], 10, 1 );

		add_filter( 'futurewordpress/project/aicontentgenerator/notices/manager', [ $this, 'noticeManager' ], 10, 3 );
		
		add_action( 'wp_ajax_futurewordpress/project/aicontentgenerator/filesystem/upload', [ $this, 'uploadFile' ], 10, 0 );
		add_action( 'wp_ajax_nopriv_futurewordpress/project/aicontentgenerator/filesystem/upload', [ $this, 'uploadFile' ], 10, 0 );
		add_action( 'wp_ajax_futurewordpress/project/aicontentgenerator/filesystem/remove', [ $this, 'removeFile' ], 10, 0 );
		add_action( 'wp_ajax_nopriv_futurewordpress/project/aicontentgenerator/filesystem/remove', [ $this, 'removeFile' ], 10, 0 );
		add_action( 'admin_post_futurewordpress/project/aicontentgenerator/filesystem/download', [ $this, 'downloadFile' ], 10, 0 );
		add_action( 'admin_post_nopriv_futurewordpress/project/aicontentgenerator/filesystem/download', [ $this, 'downloadFile' ], 10, 0 );
    ...
    return isset( FUTUREWORDPRESS_PROJECT_AICONTENTGENERATOR_OPTIONS[ $option ] ) ? FUTUREWORDPRESS_PROJECT_AICONTENTGENERATOR_OPTIONS[ $option ] : $default;
	}
	/**
    ...
    return ( isset( FUTUREWORDPRESS_PROJECT_AICONTENTGENERATOR_OPTIONS[ $option ] ) && FUTUREWORDPRESS_PROJECT_AICONTENTGENERATOR_OPTIONS[ $option ] == 'on' );
	}
	/**
	...
	public function uploadDir( $file = false, $force = false ) {
		...
		if( $this->theUploadDir === false ) {
			...
			$uploadDir[ 'basedir' ] = $uploadDir[ 'basedir' ] . '/' . apply_filters( 'futurewordpress/project/aicontentgenerator/system/getoption', 'ftp-mediadir', 'futurewordpress' );
			$uploadDir[ 'baseurl' ] = $uploadDir[ 'baseurl' ] . '/' . apply_filters( 'futurewordpress/project/aicontentgenerator/system/getoption', 'ftp-mediadir', 'futurewordpress' );
	...
	public function uploadFile() {
		check_ajax_referer( 'futurewordpress/project/aicontentgenerator/verify/nonce', '_nonce' );
		...
		if( isset( $_FILES[ 'blobFile' ] ) || isset( $_FILES[ 'file' ] ) ) {
			...
			// 	$error = sprintf( __( 'File is larger then allowed range. (%d)', 'ai-content-generator-on-acf-field' ), $file[ 'size' ][0] );
			// }
			...
			// 	$error = sprintf( __( 'File format (%s) is not allowed.', 'ai-content-generator-on-acf-field' ), $extension );
			// }
			...
			if( $error === false && move_uploaded_file( $file[ 'tmp_name' ][0], $file[ 'full_path' ] ) ) {
				...
				// $meta['type'] = apply_filters( 'futurewordpress/project/aicontentgenerator/validate/format', $meta['type'], $meta );
				$oldMeta[] = $meta;
				WC()->session->set( 'uploaded_files_to_archive', $oldMeta );
				wp_send_json_success( [
					'message'			=> __( 'Uploaded successfully', 'ai-content-generator-on-acf-field' ),
					'dropZone'		=> $meta
				], 200 );
			} else {
				$error = ( $error ) ? $error : __( 'Something went wrong while tring to upload short clip video.', 'ai-content-generator-on-acf-field' );
				wp_send_json_error( $error, 200 );
			}
		}
		wp_send_json_error( __( 'Error happens.', 'ai-content-generator-on-acf-field' ) );
	}
	public function removeFile() {
		...
		if( isset( $fileInfo[ 'full_path' ] ) && file_exists( $fileInfo[ 'full_path' ] ) && ! is_dir( $fileInfo[ 'full_path' ] ) ) {
			...
			wp_send_json_success( __( 'File removed from server.', 'ai-content-generator-on-acf-field' ), 200 );
		} else {
			wp_send_json_error( __( 'Failed to delete. Maybe File not found on server or your request doesn\'t contain file data enough.', 'ai-content-generator-on-acf-field' ), 200 );
		}
	}
	public function downloadFile() {
		...
		if( isset( $fileInfo[ 'full_url' ] ) && isset( $fileInfo[ 'full_path' ] ) && file_exists( $fileInfo[ 'full_path' ] ) && ! is_dir( $fileInfo[ 'full_path' ] ) ) {
			...
			print_r( $fileInfo );
			wp_die( __( 'File not found', 'ai-content-generator-on-acf-field' ), __( '404 not found', 'ai-content-generator-on-acf-field' ) );
		}
	}
	...
	public function dashboardPermalink( $id, $user = 'me' ) {
		...
		$profile = ( apply_filters( 'futurewordpress/project/aicontentgenerator/system/getoption', 'permalink-userby', 'id' ) == 'id' ) ? FUTUREWORDPRESS_PROJECT_DASHBOARDPERMALINK . '/' . ( ( $id ) ? $id : 'me' ) : FUTUREWORDPRESS_PROJECT_DASHBOARDPERMALINK . '/' . $user;
		return $profile . '/' . apply_filters( 'futurewordpress/project/aicontentgenerator/profile/defaulttab', 'profile' );
	}
