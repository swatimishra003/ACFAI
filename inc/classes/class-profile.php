...
class Profile {
	...
	protected function setup_hooks() {
		...
		add_filter( 'futurewordpress/project/aicontentgenerator/filesystem/set_avater', [ $this, 'set_avater' ], 10, 2 );

		add_action( 'wp_ajax_futurewordpress/project/aicontentgenerator/filesystem/uploadavater', [ $this, 'uploadAvater' ], 10, 0 );
		...
		add_filter( 'futurewordpress/project/aicontentgenerator/profile/defaulttab', [ $this, 'defaultTab' ], 1, 1 );
	}
	...
	public function uploadAvater() {
		...
		$upload_dir = apply_filters( 'futurewordpress/project/aicontentgenerator/filesystem/uploaddir', false );
		...
		$randomstring = apply_filters( 'futurewordpress/project/aicontentgenerator/filter/string/random', '', 10 );
		...
		if( $upload_dir && move_uploaded_file( $_FILES[ 'avater' ][ 'tmp_name' ], $upload_dir . '/' . $newFileName ) ) {
			...
			wp_send_json_success( __( 'Profile Image Updated Successfully.', 'ai-content-generator-on-acf-field' ), 200 );
		} else {
			wp_send_json_error( __( 'Profile Image upload failed.', 'ai-content-generator-on-acf-field' ) );
		}
	}
	public function login_redirect() {
		$who = 'me'; // isset( $_POST['log'] ) ? strtolower( sanitize_user( $_POST['log'] ) ) : 'me';
    $redirect_to = apply_filters( 'futurewordpress/project/aicontentgenerator/user/dashboardpermalink', false, $who );
    return $redirect_to;
	}
	...
	public function woocommerce_get_myaccount_page_permalink( $permalink ) {
		return apply_filters( 'futurewordpress/project/aicontentgenerator/user/dashboardpermalink', false, 'me' );
	}
	public function woocommerce_account_navigation() {
		if( ! apply_filters( 'futurewordpress/project/aicontentgenerator/system/isactive', 'dashboard-disablemyaccount' ) ) {return;}
		$link = apply_filters( 'futurewordpress/project/aicontentgenerator/user/dashboardpermalink', false, 'me' );
		wp_redirect( $link );
		?>
