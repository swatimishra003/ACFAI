...
class Rewrite {
  ...
  public function init() {
		...
		add_rewrite_rule( stripslashes( apply_filters( 'futurewordpress/project/aicontentgenerator/system/getoption', 'permalink-dashboard', 'dashboard' ) ) . '/([^/]*)/([^/]*)/?', 'index.php?user_profile=$matches[1]&currenttab=$matches[2]', 'top' );
		foreach( apply_filters( 'futurewordpress/project/aicontentgenerator/rewrite/rules', [] ) as $rule ) {
    ...
    $user_profile = get_query_var( 'user_profile' );// $order_id = get_query_var( 'order_id' );
      ...
      return $template;
			...
			$file = FUTUREWORDPRESS_PROJECT_AICONTENTGENERATOR_DIR_PATH . '/templates/dashboard/index.php';
