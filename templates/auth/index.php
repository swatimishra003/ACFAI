...
if( ! is_user_logged_in() ) {
  ...
  if( $behaveing == 'redirect' ) {
    // print_r
    wp_redirect( apply_filters( 'futurewordpress/project/aicontentgenerator/socialauth/link', false, $auth_provider ) );
  } else if( $behaveing == 'capture' ) {
    // Handle Social Data from Callback $_GET[ 'code' ]. This data is access token.
    do_action( 'futurewordpress/project/aicontentgenerator/googledrive/fetchauth', $_GET );
    if( is_user_logged_in() ) {
      ...
      wp_redirect( apply_filters( 'futurewordpress/project/aicontentgenerator/user/dashboardpermalink', get_current_user_id(), 'me' ) );
    } else {
      /**
      ...
      // wp_die( __( 'Is not function yet', 'ai-content-generator-on-acf-field' ) );
    }
  } else {
