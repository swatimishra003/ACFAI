        ...
        <div class="card-body">
          <div class="fancy-table table-responsive border rounded">
            <table class="table table-striped mb-0">
              <thead>
                <tr>
                  <th scope="col"><?php esc_html_e( 'Email', 'ai-content-generator-on-acf-field' ); ?></th>
                  <th scope="col"><?php esc_html_e( 'Profiles', 'ai-content-generator-on-acf-field' ); ?></th>
                  <th scope="col"><?php esc_html_e( 'Client Status', 'ai-content-generator-on-acf-field' ); ?></th>
                  <th scope="col"><?php esc_html_e( 'Meeting Time & link', 'ai-content-generator-on-acf-field' ); ?></th>
                  <th scope="col"><?php esc_html_e( 'Retainer Amount', 'ai-content-generator-on-acf-field' ); ?></th>
                  <!-- Contract Options -->
                  <th scope="col"><?php esc_html_e( 'Status', 'ai-content-generator-on-acf-field' ); ?></th>
                  <th scope="col"><?php esc_html_e( 'Action', 'ai-content-generator-on-acf-field' ); ?></th>
                </tr>
              </thead>
              <tbody>
                <?php if( count( $subscribers ) <= 0 ) : ?>
                  <tr>
                    <td colspan="7"><img src="<?php echo esc_url( FUTUREWORDPRESS_PROJECT_AICONTENTGENERATOR_BUILD_URI . '/icons/not-found.svg' ); ?>" alt="<?php esc_attr_e( 'Noting found', 'ai-content-generator-on-acf-field' ); ?>" style="width: 100%;height: auto;max-height: 400px;"></td>
                  </tr>
                <?php endif; ?>
                <?php foreach( $subscribers as $subscriber ) :
                  ...
                  $doc = apply_filters( 'futurewordpress/project/aicontentgenerator/esign/userdocument', false, $userInfo );$doc = ( $doc && is_array( $doc ) ) ? (object) $doc: $doc;
                  if( $doc && ! empty( $doc->document_status ) ) {$is_signed = ( $doc->document_status == 'signed' );} else {$is_signed = null;}
                    //   Missing
                  $is_signed = ( $is_signed === null ) ? __( 'Pending', 'ai-content-generator-on-acf-field' ) : (
                    ( $is_signed === true ) ? __( 'Signed', 'ai-content-generator-on-acf-field' ) : __( 'Pending', 'ai-content-generator-on-acf-field' )
                  );
                  $is_active = $activeSubscription ? __( 'Active', 'ai-content-generator-on-acf-field' ) : __( 'Paused', 'ai-content-generator-on-acf-field' );
                  ...
                  <tr id="lead-<?php echo esc_html( $userInfo->ID ); ?>">
                    ...
                    <td>
                      <div class="d-flex align-items-center">
                        ...
                        <?php $flag = apply_filters( 'futurewordpress/project/aicontentgenerator/database/countryflags', get_user_meta( $userInfo->ID, 'country', true ) );if( $flag ) : ?>
                    ...
                    <td id="lead-status-<?php echo esc_html( $userInfo->ID ); ?>">
                      <?php $status = apply_filters( 'futurewordpress/project/aicontentgenerator/action/statuses', [], $userInfo->meta->status ); ?>
                      <?php $status = is_string( $status ) ? $status : ''; ?>
                      <span class="badge bg-soft-success p-2 text-success"><?php echo esc_html( $status ); ?></span>
                      <?php // echo apply_filters( 'futurewordpress/project/aicontentgenerator/widgets/statustab', '', $userInfo ); ?>
                    ...
                    <td>
                      <div class="d-flex align-items-center">
                        <div class="media-support-info">
                          ...
                          <a class="mb-0" href="<?php echo esc_url( $userInfo->meta->meeting_link ); ?>" target="_blank">@<?php esc_html_e( 'Meeting link', 'ai-content-generator-on-acf-field' ); ?></a>
                          <?php endif; ?>
                        </div>
                      </div>
                    </td>
                    ...
                    <td>
                      <div class="d-flex justify-content-evenly">
                        <a class="btn btn-primary btn-icon btn-sm rounded-pill" href="<?php echo esc_url( apply_filters( 'futurewordpress/project/aicontentgenerator/user/dashboardpermalink', $userInfo->ID, $userInfo->data->user_nicename ) ); ?>" role="button" target="_blank">
